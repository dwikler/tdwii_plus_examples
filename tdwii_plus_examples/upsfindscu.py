#!/usr/bin/env python
"""A UPS C-FIND SCU application.

Used for uploading DICOM UPS SOP Instances to a UPS SCP.
(based on findscu.py from pynetdicom adding support for TLS)
"""
...
def _setup_argparser(args=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "(SCU) for the Unified Procedure Step Service Class."
            "findscu only supports query functionality using the C-FIND "
...
def main(args=None):
    ...
    if any(ext_opts):
        ...
        item.sop_class_uid = UnifiedProcedureStepPull
        item.service_class_application_information = app_info
        ext_neg = [item]
    elif any([args.fuzzy_names, args.timezone_adj]):
        app_info = b"\x01\x01"
    ...
    if assoc.is_established:
        ...
        responses = assoc.send_c_find(identifier, UnifiedProcedureStepPull)
        ...
        for status, rsp_identifier in responses:
            ...
            if status and status.Status in [0xFF00, 0xFF01]:
                if args.write:
                    rsp_identifier.file_meta = get_file_meta(assoc, UnifiedProcedureStepPull)
